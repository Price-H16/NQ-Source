...
using ConfigurationHelper = NosQuest.World.Configuration.ConfigurationHelper;
...
";
        ...
        private static void UnhandledExceptionHandler(object sender, UnhandledExceptionEventArgs e)
        {
            ...
            ServerManager.Instance.DisconnectAll();
            var a = DependencyContainer.Instance.GetInstance<JsonGameConfiguration>().Server;
            Process.Start("NosQuest.World.exe", $"--nomsg --port {(ServerManager.Instance.ChannelId == 51 ? $"{a.Act4Port}" : $"{_port}")}");
            Environment.Exit(1);
        }
        #endregion
    }
