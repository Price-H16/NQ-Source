...
namespace OpenNos.Handler.PacketHandler.Npc
{
    public class MShopPacketHandler : IPacketHandler
    {
        ...
        public void CreateShop(MShopPacket packet)
        {
            ...
            if (!short.TryParse(packetsplit[0], out var typePacket)) return;
                ...
                case 0:
                    ...
                    if (packetsplit.Length > 82)
                    {
                        ...
                        for (short j = 1, i = 0; j < 80; j += 4, i++)
                        {
                            ...
                            if (qty[i] > 0)
                            {
                                var inv = Session.Character.Inventory.LoadBySlotAndType(slot[i], type[i]);
                                if (inv != null)
                                {
